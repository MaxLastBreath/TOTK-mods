...
def checkpath(self, mode):
    ...
    if self.os_platform == "Linux":
        if mode == "Yuzu":
            ...
            self.load_dir = os.path.normpath(config_parser.get('Data%20Storage', 'load_directory', fallback=f'No Config'))
            if self.load_dir == "No Config":
                self.nand_dir = os.path.normpath(os.path.join(self.Globaldir, "nand"))
                self.load_dir = os.path.normpath(os.path.join(self.Globaldir, "load"))
